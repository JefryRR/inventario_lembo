...
export default function TypesCreate() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const data = await apiFetch("tipo_insumos/crear", {
                method: "POST",
                body: payload,
            });

            setSuccess(data?.message || "Tipo de insumo creado correctamente");
            setForm(initialState);
            navigate("/tipos-insumos");
        } catch (requestError: any) {
            setError(
                requestError?.detail || requestError?.message || "Ocurrió un error al crear el tipo de insumo"
            );
        } finally {
            setLoading(false);
        }
                ...
                <div className="flex flex-col gap-2 border-b border-gray-200 px-5 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Completa los datos obligatorios para registrar el tipo de insumo.
                        </p>
                    </div>
